...
class BaseRecognition:
    ...
    def run(self):
        ...
        if self.session is None:
            # ИСПРАВЛЕНИЕ: Явно указываем провайдеры
...
class Runner:
    ...
    def add_frame(self, image):
        ...
        if self.frame_counter == self.frame_interval:
            # ОГРАНИЧЕНИЕ ОЧЕРЕДИ: не допускаем переполнения
            if len(self.tensors_list) > 48:  # window_size * 3 = 16 * 3 = 48
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            # Проверяем, готов ли распознаватель
            if self.multiprocess:
                if not self.recognizer.started.value:
                    time.sleep(0.01)
                    continue
            else:
                if not self.recognizer.started:
                    continue
            ...
            if self.prediction_list:
                # Берем только последние length предсказаний
                recent_predictions = list(self.prediction_list)[-self.length:]
                text = "  ".join(recent_predictions)
                cv2.putText(text_div, text, (10, 30), cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 255), 2)

            # Ограничиваем длину списка предсказаний
            if len(self.prediction_list) > self.length * 2:
                while len(self.prediction_list) > self.length * 2:
